...
const api: DesktopAPI = {
  ...
  createDocument: (workspaceRoot, path, content, bom) => ipcRenderer.invoke('slx:create', { workspaceRoot, path, content, bom }),
  ...
  startModelEdit: (path, edit) => ipcRenderer.invoke('slx:modelEditStart', { path, edit }),
  startSimulation: (path, expectedSha256, stopTime) => ipcRenderer.invoke('slx:simulationStart', { path, expectedSha256, stopTime }),
  modelJobStatus: jobId => ipcRenderer.invoke('slx:modelJobStatus', { jobId }),
  stopModelJob: jobId => ipcRenderer.invoke('slx:modelJobStop', { jobId }),
  modelHistory: (path, action = 'status') => ipcRenderer.invoke('slx:modelHistory', { path, action }),
  ...
  matlabSetVariable: (name, expression) => ipcRenderer.invoke('slx:matlabVariableSet', { name, expression }),
  matlabResultPage: (kind, jobId, options = {}) => ipcRenderer.invoke('slx:matlabResultPage', { kind, jobId, ...options }),
  ...
  extensionsRestart: id => ipcRenderer.invoke('slx:extensionsRestart', { id }),
  ...
  onExtensionState: callback => subscribe('slx:extensionState', callback),
