...
def _text(value: Any, *, field: str, max_chars: int = _MAX_VALUE_CHARS) -> str:
    if not isinstance(value, str):
        raise ValueError(f"blueprint field {field!r} must be a string")
...
def _position(value: Any, *, field: str) -> tuple[int, int, int, int]:
    ...
    nums = tuple(int(round(float(x))) for x in value)
...
def _params(value: Any, *, block_type: str) -> dict[str, str]:
    ...
    if not isinstance(value, dict):
        raise ValueError("block parameters must be an object")
...
def blueprint_from_dict(payload: dict[str, Any]) -> ModelBlueprint:
    ...
    for index, raw in enumerate(raw_blocks):
        if not isinstance(raw, dict):
            raise ValueError(f"blueprint block {index} must be an object")
    ...
    if not isinstance(raw_connections, list):
        raise ValueError("blueprint connections must be an array")
    ...
    for index, raw in enumerate(raw_connections):
        if not isinstance(raw, dict):
            raise ValueError(f"blueprint connection {index} must be an object")
