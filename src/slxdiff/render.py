...
def render_markdown(result: DiffResult, *, heading_level: int = 2) -> str:
    h = "#" * heading_level
    status = (
        "✅ No semantic changes" if not result.changed else f"🔎 {result.change_count} semantic change(s)"
    )
    out = [
    ...
    if result.changed_blocks:
        ...
        for change in result.changed_blocks:
            ...
            if change.type_changed:
                out.append(f"  - type: {_code(change.before.block_type)} → {_code(change.after.block_type)}")
            for param in change.parameter_changes:
...
def render_git_markdown(diffs) -> str:
    ...
    out = [
        "## slx-diff review",
        "",
        (f"**{_count_label(len(diffs), 'model')} inspected · {_count_label(total, 'semantic change')}**"),
        "",
    ...
    for item in diffs:
        ...
        out.append(
            f"| {_code(item.display_path)} | `{item.status}` | {item.result.change_count} | {review_label} |"
        )
    ...
    for item in diffs:
        ...
        if item.review.items:
            ...
            out.extend(
                [
                    "",
                    "> Static review-priority heuristic only; this is not a safety, stability, or simulation verdict.",
                    "",
                ]
            )
        out.append("</details>")
